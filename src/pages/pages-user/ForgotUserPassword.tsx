...
const ForgotUserPassword = () => {
    ...
    const handleSubmit = (values: { email: string; password: string; otp?: string }) => {
        if (showForgot) {
            axios.post('http://localhost:3000/api/user/forgot-password', { email: values.email, password: values.password },{withCredentials:true})
                .then(response => {
            ...
            axios.post('http://localhost:3000/api/user/verify-forgototp', { email: values.email, otp: values.otp, },{withCredentials:true})
                .then(response => {
                    if (response.data.status) {
